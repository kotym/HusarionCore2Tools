...
static int set_uart_baudrate(int requestedSpeed)
{
	if (!ftdi)
		return -1;

#ifdef WIN32
	// Newer libftdi Windows builds expect the real baud value. Older builds
	// sometimes needed x4 scaling; keep it as a fallback only.
	int ret = ftdi_set_baudrate(ftdi, requestedSpeed);
	if (ret < 0)
	{
		ret = ftdi_set_baudrate(ftdi, requestedSpeed * 4);
	}
	return ret;
#else
	return ftdi_set_baudrate(ftdi, requestedSpeed);
#endif
}
...
bool uart_open(int speed, bool showErrors)
{
	...
	if (set_uart_baudrate(speed) < 0)
	{
		if (showErrors)
			fprintf(stderr, "unable to set baudrate %d (%s)\n", speed, ftdi_get_error_string(ftdi));
		uart_close();
		return false;
	}
