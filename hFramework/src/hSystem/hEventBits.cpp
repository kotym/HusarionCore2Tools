...
{
	return (uint32_t)xEventGroupWaitBits(this->impl->eventGroupHandle, (EventBits_t)(bitMask & 0x00FFFFFF), pdFALSE, pdTRUE, (TickType_t)timeout);
}

uint32_t hEventBits::waitAny(uint32_t bitMask, uint32_t timeout)
{
	return (uint32_t)xEventGroupWaitBits(this->impl->eventGroupHandle, (EventBits_t)(bitMask & 0x00FFFFFF), pdFALSE, pdFALSE, (TickType_t)timeout);
}

uint32_t hEventBits::setBits(uint32_t bitsToSet)
{
	return (uint32_t)xEventGroupSetBits(this->impl->eventGroupHandle, (EventBits_t)(bitsToSet & 0x00FFFFFF));
}

uint32_t hEventBits::clearBits(uint32_t bitsToClear)
{
	return (uint32_t)xEventGroupClearBits(this->impl->eventGroupHandle, (EventBits_t)(bitsToClear & 0x00FFFFFF));
}

uint32_t hEventBits::clear()
{
	return (uint32_t)xEventGroupClearBits(this->impl->eventGroupHandle, (EventBits_t)0x00FFFFFF);
}
